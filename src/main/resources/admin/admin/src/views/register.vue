  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(183, 174, 174, 0.5);borderRadius:22px">
        <h1 class="h1" style="color:#000;fontSize:28px;">古典舞在线交流平台注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			<!-- <div v-if="tableName=='huiyuanyonghu'" class="input-group">
			   <div class="label">账号</div>
			   <div class="input-container">
			     <input v-model="ruleForm.zhanghao" class="input" type="text" placeholder="账号">
			   </div>
			 </div> -->
			<el-form-item label="账号" class="input" v-if="tableName=='huiyuanyonghu'">
			...
			<el-form-item label="密码" class="input" v-if="tableName=='huiyuanyonghu'">
			...
			<el-form-item label="姓名" class="input" v-if="tableName=='huiyuanyonghu'">
			...
			<el-form-item label="手机" class="input" v-if="tableName=='huiyuanyonghu'">
			...
			<el-form-item label="邮箱" class="input" v-if="tableName=='huiyuanyonghu'">
			...
			<el-form-item label="身份证" class="input" v-if="tableName=='huiyuanyonghu'">
...
export default {
  ...
  methods: {
    ...
    // 注册
    login() {
      if((!this.ruleForm.zhanghao) && `huiyuanyonghu` == this.tableName){
        this.$message.error(`账号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `huiyuanyonghu` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      if(`huiyuanyonghu` == this.tableName && this.ruleForm.shouji&&(!this.$validate.isMobile(this.ruleForm.shouji))){
        this.$message.error(`手机应输入手机格式`);
        return
      }
      if(`huiyuanyonghu` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮件格式`);
        return
      }
      if(`huiyuanyonghu` == this.tableName && this.ruleForm.shenfenzheng&&(!this.$validate.checkIdCard(this.ruleForm.shenfenzheng))){
        this.$message.error(`身份证应输入身份证格式`);
        return
      }
      if(`huiyuanyonghu` == this.tableName && this.ruleForm.money&&(!this.$validate.isNumber(this.ruleForm.money))){
...
<style lang="scss" scoped>
 ...
 //  }
	...
	.container {
		background-image: url("http://codegen.caihongy.cn/20210124/350774c74b434010bca42a2e6158355d.jpg");
		height: 100vh;
